...
| 항목               | 내용                                                                                                       |
| ------------------ | ---------------------------------------------------------------------------------------------------------- |
| **One-liner**      | 지금 시작할 수 있는 부업 정보를 한눈에                                                                     |
...
- [x] 홈(`/`) 로드 시 3개 섹션이 순서대로 표시된다: 🔥 뜨는 부업 → 💡 많이 찾는 부업 → 📚 모든 부업
- [x] 뜨는 부업 섹션은 `isTrending: true` 항목만 표시한다 (최대 4개)
- [x] 많이 찾는 부업 섹션은 `isPopular: true` 항목만 표시한다 (최대 4개)
- [x] 모든 부업 섹션은 전체 항목을 `trendScore` 내림차순으로 표시한다
- [x] 각 카드는 아이콘 / 난이도·특성 배지 / 제목 / 1줄 요약 / 수익 범위 / 태그를 포함한다
- [x] 카드 클릭 시 `/side-hustle/[slug]`로 이동한다
- [x] 데스크톱(1024px+): 뜨는/인기 섹션 2열, 모든 부업 섹션 3열 그리드
- [x] 모바일(767px 이하): 모든 섹션 1열 그리드
...
- [x] TagFilterBar의 태그 클릭 시 URL이 `/?tag=[tagname]`으로 업데이트된다 (페이지 새로고침 없음)
- [x] URL `/?tag=무자본`으로 직접 접속 시 필터가 자동 적용된 상태로 렌더링된다
- [x] 태그 필터 적용 시 3개 섹션 모두 해당 태그를 가진 항목만 표시한다
- [x] 필터링 결과가 0개인 섹션은 숨긴다
- [x] 3개 섹션 모두 결과가 0개이면 "해당 태그의 부업 정보를 준비 중입니다" 메시지를 표시한다
- [x] 현재 선택된 태그 칩은 활성화 스타일(배경색 강조)을 표시한다
- [x] 동일 태그 재클릭 시 필터 해제, URL은 `/`로 복귀한다
- [x] MVP에서 태그 단일 선택만 지원한다 (다중 선택 Out of Scope)
...
- [x] 모든 부업 상세 페이지는 빌드 타임에 정적으로 생성된다 (`generateStaticParams`)
- [x] 존재하지 않는 slug 접근 시 Next.js 404 페이지를 반환한다 (`notFound()`)
- [x] MetricPanel은 4개 지표를 표시한다: 난이도 / 초기 비용 / 예상 월 수익 / 첫 수입까지 기간
- [x] StartGuide는 step 번호 순서대로 모든 단계를 표시한다 (최소 3단계)
- [x] `<title>` 태그: `[title] 부업 가이드 | 부업레이더` 형식
- [x] `<meta description>`: summary 필드 기반, 160자 이내
- [x] OG 태그 (`og:title`, `og:description`)가 각 페이지 고유하게 설정된다
- [x] 상세 페이지에서 홈으로 돌아가는 링크가 존재한다
...
- [x] AdSlot 컴포넌트는 `slotId`가 빈 문자열(`""`)일 때 회색 placeholder를 고정 크기로 렌더링한다
- [x] AdSlot 컨테이너는 slotId 유무와 무관하게 항상 규격 높이를 유지한다 (CLS 방지)
- [x] 사이드바 AdSlot(300×250, 300×600)은 데스크톱에서만 표시된다
- [x] 모바일 배너 AdSlot(320×100)은 모바일에서만 표시된다
- [x] AdSense 스크립트는 `next/script strategy="afterInteractive"`로 삽입된다 (LCP 차단 방지)
